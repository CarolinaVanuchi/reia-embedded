...
{
    if (error_code != 0) {
        ESP_LOGE(__FILE__, "Last error %s: 0x%x", message, error_code);
    }
}

static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ESP_LOGD(__FILE__, "Event dispatched from event loop base=%s, event_id=%d", base, event_id);
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        ESP_LOGI(__FILE__, "sent publish successful, msg_id=%d", msg_id);
        break;
    ...
    case MQTT_EVENT_ERROR:
        ...
        if (event->error_handle->error_type == MQTT_ERROR_TYPE_TCP_TRANSPORT) {
            ...
            ESP_LOGI(__FILE__, "Last errno string (%s)", strerror(event->error_handle->esp_transport_sock_errno));
...
{
    ESP_LOGI(__FILE__, "[APP] Startup..");
    ESP_LOGI(__FILE__, "[APP] Free memory: %d bytes", esp_get_free_heap_size());
    ESP_LOGI(__FILE__, "[APP] IDF version: %s", esp_get_idf_version());
